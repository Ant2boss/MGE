...
MGEngine::GUI_ButtonList& MGEngine::GUI_ButtonList::SetButtonColor(MGEngine::uint ButtonIndex, MGEngine::uint Color) {
	if (ButtonIndex >= this->GetButtonCount()) return *this;
	this->MyButtonArray[ButtonIndex].SetColor(Color);
	return *this;
}

MGEngine::GUI_ButtonList& MGEngine::GUI_ButtonList::SetButtonTextColor(MGEngine::uint ButtonIndex, MGEngine::uint Color) {
	if (ButtonIndex >= this->GetButtonCount()) return *this;
	this->MyButtonArray[ButtonIndex].SetTextColor(Color);
	return *this;
}

MGEngine::GUI_ButtonList& MGEngine::GUI_ButtonList::SetButtonSelectedColor(MGEngine::uint ButtonIndex, MGEngine::uint Color) {
	if (ButtonIndex >= this->GetButtonCount()) return *this;
...
std::string MGEngine::Canvas::loadUserInput() {
	...
	while (1) {
		...
		while (1) {
			...
			else if (this->GetMouseLeftClickState()) {
				MosuePressed = true;
				break;
			}
		}
...
MGEngine::uint MGEngine::Canvas::GetTextColor() {
...
MGEngine::Canvas& MGEngine::Canvas::operator<<(const MGEngine::Color Val) {
	this->SetTextColor(Val.IgnoreMe);
	return *this;
}
...
MGEngine::Color MGEngine::SetTextColor(uint ColorVal) {
	MGEngine::Color C;
	C.IgnoreMe = ColorVal;
	return C;
}
...
void MGEngine::PlaceTextBlock(MGEngine::Canvas& MyCan, std::string Text, MGEngine::uint Color, MGEngine::Point2i Pos) {
	...
	MyCan.SetTextColor(OriginalColor);
}
